...
for index, row in metadata.iterrows():
    ...
    data.append([mri_post, mri_right, mri_left, info, row["category"], row["number"], row["isReal"], row["origin"]])


dataframe = pd.DataFrame(data, columns=["mri_post", "mri_right", "mri_left", "info", "category", "number", "isReal", "origin"])
...
# # split the dataset 
real_train, real_test = train_test_split(real_data_frame, test_size=0.2)
train = pd.DataFrame(columns=["mri_post", "mri_right", "mri_left", "info", "category", "number", "isReal", "origin"])
for row in real_train.iloc():
    data = dataframe[(dataframe["category"] == row["category"]) & (dataframe["origin"] == row["number"])]
    train = train.append(data, ignore_index=True)
test = pd.DataFrame(columns=["mri_post", "mri_right", "mri_left", "info", "category", "number", "isReal", "origin"])
for row in real_test.iloc():
    data = dataframe[(dataframe["category"] == row["category"]) & (dataframe["origin"] == row["number"])]
    test = test.append(data, ignore_index=True)

print("len of Training Data:", len(train), " len of Testing Data:", len(test))
...
# tbCallBack = TensorBoardts=1024, activation='relu')(x)
    # x = Dropout(0.2)(x)
    # x = Dense(units=1024, activation='relu')(x)
    # x = Dropout(0.2)(x)(log_dir=logs_path,
#                  histogram_freq=0,
#                  write_graph=True, 
#                  write_grads=True, 
#                  write_images=True,
...
# history = model.fit([post_train_X, right_train_X, left_train_X, info_train_x], 
#                     train_y, 
#                     batch_size=num_batch_size, 
#                     epochs=num_epochs, 
#                     validation_data=([post_train_X, right_train_X, left_train_X, info_train_x], train_y), verbose=1)
# four_history_list.append(history)
# four_models.append(model)
# # model.save_weights(filepath=weight_path)
# score = model.evaluate([post_test_X, right_test_X, left_test_X, info_test_X], test_y, verbose=1, batch_size=num_batch_size)
...
# model = get_3model()
# history = model.fit([post_train_X, right_train_X, left_train_X], 
#                     train_y, 
#                     batch_size=num_batch_size, 
#                     epochs=num_epochs, 
#                     validation_data=([post_train_X, right_train_X, left_train_X], train_y), verbose=1)
# three_history_list.append(history)
# three_models.append(model)
# score = model.evaluate([post_test_X, right_test_X, left_test_X], test_y, verbose=1, batch_size=num_batch_size)
# four_score.append(score)
...
# train_count = np.count_nonzero(train_y, axis=0)
# test_count = np.count_nonzero(train_y, axis=0)
# print("Train count[Af, TS]:", train_count)
# print("Validation count[Af, TS]:", test_count)
# %%
# TO PLOT ROC CURVE
